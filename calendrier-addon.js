/* EuroBet Calendriers loader v6c7 — gzip UTF-8 via jsDelivr (Pages 404s *.gz.b64 app parts) */
(function(){
  const CDN="https://cdn.jsdelivr.net/gh/delestrestd/EUROBET@main/";
  ...
  async function loadGzParts(prefix,n){
    const texts=await Promise.all([...Array(n).keys()].map(async i=>{
      const r=await fetch(CDN+prefix+".part"+i+"?v=6c7",{cache:"no-cache"});
  ...
  async function boot(){
    (0,eval)(await loadGzParts("calendrier-addon-app.js.gz.b64",14));
    (0,eval)(await loadGzParts("calendrier-v6c5.js.gz.b64",9));
  }
