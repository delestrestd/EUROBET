/* EuroBet Calendriers loader v6c7 — UTF-8 .js chunks (Pages-safe, no mojibake) */
(function(){
  ...
  async function loadChunks(stem,n){
    const texts=await Promise.all([...Array(n).keys()].map(async i=>{
      const r=await fetch("./"+stem+"-"+i+".js?v=6c7",{cache:"no-cache"});
      if(!r.ok)throw new Error(stem+"-"+i+".js HTTP "+r.status);
      return await r.text();
    }));
    return texts.join("");
  }
  async function boot(){
    (0,eval)(await loadChunks("cal-app",5));
    (0,eval)(await loadChunks("cal-patch",3));
  }
